...
/* ── I2C handles (module-level, single bus/device) ──────────────── */
...
{
    /* Build buffer: control byte 0x40 + data */
    ...
    esp_err_t ret = i2c_master_transmit(s_i2c_dev, buf, 1 + len,
                                        I2C_TIMEOUT_MS);
    free(buf);
    return ret;
}
...
esp_err_t ssd1306_init(ssd1306_t **out_dev)
{
    ...
    ret = i2c_master_probe(s_i2c_bus, OLED_I2C_ADDR, I2C_TIMEOUT_MS);
    if (ret != ESP_OK) {
...
esp_err_t ssd1306_update(ssd1306_t *dev)
{
    ...
    ret = oled_write_cmd(0x00);  if (ret != ESP_OK) return ret;  /* Start: 0 */
    ret = oled_write_cmd(0x7F);  if (ret != ESP_OK) return ret;  /* End: 127 */
    ret = oled_write_cmd(0x22);  if (ret != ESP_OK) return ret;  /* Page addr */
    ret = oled_write_cmd(0x00);  if (ret != ESP_OK) return ret;  /* Start: 0 */
    ret = oled_write_cmd(0x07);  if (ret != ESP_OK) return ret;  /* End: 7 */

    /* Send framebuffer page by page (128 bytes each) */
    for (int p = 0; p < OLED_PAGES; p++) {
        ret = oled_write_data(&dev->framebuf[p * OLED_WIDTH], OLED_WIDTH);
        if (ret != ESP_OK) {
